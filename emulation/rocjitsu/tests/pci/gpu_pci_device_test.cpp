...
constexpr uint64_t kGfx12SystemLeafPteFlags = (uint64_t{1} << 63) | 0x63;
...
TEST_F(GpuDevice, PublishesVmidZeroBeforeAcknowledgingItsInvalidation) {
  ...
  // amdgpu_gmc_pd_addr() stores the page-table address together with the
  // GFX12 valid and snooped PDE flags in these registers.
  constexpr uint64_t kEncodedPageTable = kPageTable | 0x5;
  constexpr uint64_t kGartStart = 0x100000000ULL;
  write_register_at(absolute(0x169f), static_cast<uint32_t>(kEncodedPageTable));
  write_register_at(absolute(0x16a0), static_cast<uint32_t>(kEncodedPageTable >> 32));
  ...
  const uint64_t published_epoch = soc_.gpu_vm().lookup(gart)->translation_epoch;
  EXPECT_EQ(published_epoch, old_epoch + 1);
  ...
  // A later failed publication must withdraw this request's old acknowledge.
  // Otherwise the guest can mistake a stale success bit for this flush
  // completing against the invalid replacement root.
  write_register_at(absolute(0x16df), 0);
  write_register_at(absolute(0x16e0), 0);
  write_register_at(absolute(0x1657 + 17), 1);

  EXPECT_EQ(read_register_at(0xa368) & 1u, 0u);
  ASSERT_TRUE(soc_.gpu_vm().lookup(gart));
  EXPECT_EQ(soc_.gpu_vm().lookup(gart)->translation_epoch, published_epoch);
}
...
TEST(VramStoreAtomics, PerformsStrongAlignedOperationsWithoutReadWriteSynthesis) {
  ...
  const auto loaded16 = vram.atomic_load(0x100, sizeof(uint16_t));
  ASSERT_TRUE(loaded16.has_value());
  ...
  ASSERT_TRUE(vram.atomic_store(0x1100, sizeof(uint32_t), 7));
  const auto fallback_exchange =
      vram.compare_exchange(0x1100, sizeof(uint32_t), 0x100000007ULL, 11);
  ASSERT_TRUE(fallback_exchange.has_value());
  EXPECT_EQ(fallback_exchange->observed, 7u);
  EXPECT_TRUE(fallback_exchange->exchanged);
  const auto fallback_loaded = vram.atomic_load(0x1100, sizeof(uint32_t));
  ASSERT_TRUE(fallback_loaded.has_value());
  EXPECT_EQ(fallback_loaded->value, 11u);
...
protected:
  ...
  void configure_gart() {
    ...
    for (const auto &[page, physical] :
      ...
      const uint64_t pte = physical | kGfx12SystemLeafPteFlags;
  ...
  submit_sdma_queue(uint32_t process_id, uint64_t doorbell_offset, uint64_t ring_gpu,
    ...
    auto mes_doorbell = std::bit_cast<std::array<std::byte, sizeof(uint64_t)>>(uint64_t{64});
    ring_doorbell(0x60, mes_doorbell);
    return soc_.mes_engine().process_address_space_for_test(process_id);
  }
};
...
TEST_F(GpuDeviceMes, InvalidatingAPasidDoesNotTargetAnUnownedRoutedVmid) {
  ...
  EXPECT_EQ(soc_.gpu_vm().lookup(address_space)->translation_epoch, old_epoch);
  EXPECT_EQ(transport_.dword_at(kApiStatusPhysical), 0u);
}
...
TEST_F(GpuDeviceMes, UpdatingRootPreservesAddressSpaceIdentityAndRedirectsExistingQueues) {
  ...
  const auto program_root = [&](uint64_t root, uint64_t pdb2, uint64_t pdb1, uint64_t pdb0,
    ...
    store_vram_qword(ptb + 0x10 * sizeof(uint64_t), physical | kGfx12SystemLeafPteFlags);
  };
  ...
  const std::optional<rocjitsu::amdgpu::AddressSpaceHandle> address_space =
      soc_.mes_engine().process_address_space_for_test(kPasid);
  ASSERT_TRUE(address_space);
  const uint64_t old_epoch = soc_.gpu_vm().lookup(*address_space)->translation_epoch;
...
TEST_F(GpuDeviceMes, ExecutesTheFirmwareFreePm4ComputeRing) {
  ...
  store_vram_qword(kProcessPtb + 0x10 * sizeof(uint64_t),
                   kComputeRingPhysical | kGfx12SystemLeafPteFlags);
  store_vram_qword(kProcessPtb + 0x11 * sizeof(uint64_t),
                   kComputeSecondPagePhysical | kGfx12SystemLeafPteFlags);
...
TEST_F(GpuDeviceMes, SdmaToComputeSameDoorbellReplacesThePriorBinding) {
  ...
  for (const auto &[page, physical] :
    ...
    store_vram_qword(kProcessPtb + page * sizeof(uint64_t), physical | kGfx12SystemLeafPteFlags);
  }
  ...
  const std::optional<rocjitsu::amdgpu::AddressSpaceHandle> original_address_space =
      soc_.mes_engine().process_address_space_for_test(3);
  ...
  EXPECT_EQ(soc_.mes_engine().process_address_space_for_test(3), original_address_space);
...
TEST_F(GpuDeviceMes, LostConnectionReleasesOnlyPciQueuesAndAllowsPasidReuse) {
  ...
  EXPECT_FALSE(soc_.mes_engine().process_address_space_for_test(kPasid));
  ASSERT_TRUE(soc_.gpu_vm().lookup(gart));
...
TEST(GpuDeviceLifetime, ReplacementFrontendReusesPasidAndDoorbellAfterDestruction) {
  ...
  const auto configure_and_add_queue = [&](rocjitsu::GpuPciDevice &device) {
    ...
    for (const auto &[page, physical] :
      ...
      if (!write_vram_qword(device, kPageTable + page * sizeof(uint64_t),
                            physical | kGfx12SystemLeafPteFlags))
        return std::optional<rocjitsu::amdgpu::AddressSpaceHandle>{};
    }
    ...
    return soc.mes_engine().process_address_space_for_test(kPasid);
  };
  ...
  EXPECT_FALSE(soc.mes_engine().process_address_space_for_test(kPasid));
...
TEST_F(GpuDeviceSdma, RegisterBackedQueueRoutesAddressesOutsideTheGartApertureToVram) {
  constexpr uint64_t kSdmaBase = 0x1260;
  ...
  // VMID 0 routes the programmed GART aperture through system-memory PTEs and
  // treats addresses outside it as local-memory physical offsets.
...
TEST_F(GpuDeviceSdma, AppliesEveryRegisterPollComparisonInTheSdmaPacketProcessor) {
  ...
  ring_doorbell(0x800, doorbell);
  ASSERT_TRUE(device_.synchronize_sdma_queue_for_test(/*engine=*/0));
...
TEST_F(GpuDeviceDelivery, SerializesDiagnosticAndCompletionInterruptsOnThePciOwnerThread) {
  std::atomic<bool> diagnostic_accepted = false;
  std::jthread diagnostic([&] {
    diagnostic_accepted =
        device_.request_interrupt({.client_id = 0x1a, .source_id = 0x00, .data = {0x11111111}});
  });
  std::jthread completion([&] { device_.deliver_cp_interrupt_for_test(7, 0x22222222); });
  diagnostic.join();
  completion.join();

  ASSERT_TRUE(diagnostic_accepted.load());
  EXPECT_TRUE(transport_.triggered().empty())
      << "neither external producer may publish the multi-step ring transaction";

  device_.drain_doorbell_inbox_for_test();

  ASSERT_EQ(transport_.triggered().size(), 2u);
  EXPECT_EQ(transport_.dword_at(kWptrAddress), 2 * kInterruptEntryBytes);
  const uint32_t first = transport_.dword_at(kRingBase) & 0xffffu;
  const uint32_t second = transport_.dword_at(kRingBase + kInterruptEntryBytes) & 0xffffu;
  EXPECT_TRUE((first == 0x001au && second == 0xb514u) || (first == 0xb514u && second == 0x001au))
      << "both requests must occupy distinct entries regardless of producer order";
}
