...
#define ROCJITSU_COMPLETION_TSAN 1
...
#ifdef ROCJITSU_COMPLETION_TSAN
  ...
  for (auto &qs : queues) {
    ...
    while (!qs.entries.empty() && qs.entries.front().fully_completed()) {
      ...
      if (!entry.completion_notified)
        break;
      if (dispatch_retirement_gate_) {
        const VmAccessOutcome retirement = dispatch_retirement_gate_(qs, entry);
        if (retirement == VmAccessOutcome::Unavailable) {
          qs.publication_retry_pending = true;
          result.retry_pending = true;
          publication_stalled = true;
          break;
        }
        if (retirement != VmAccessOutcome::Complete) {
          entry.terminal_faulted = true;
          if (entry.grid_completion)
            entry.grid_completion->mark_faulted();
          result.terminal_fault = CompletionDrainFault{
...
#ifdef ROCJITSU_COMPLETION_TSAN
          // ROCR's matching scacquire wait lives in an uninstrumented shared
          // library. Publish the same edge explicitly so TSan can follow GPU
          // worker writes through completion to the waiting application.
          __tsan_release(reinterpret_cast<void *>(entry.completion_signal + kSignalValueOffset));
#endif
