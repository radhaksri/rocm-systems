...
/// @brief Whether a queue layout can be admitted without a terminal service failure.
[[nodiscard]] bool valid_sdma_ring_config(const SdmaRingConfig &config);
...
public:
  ...
  void reset();
  /// @brief Convert an escaped packet callback into durable terminal queue state.
  void latch_callback_failure() noexcept;
