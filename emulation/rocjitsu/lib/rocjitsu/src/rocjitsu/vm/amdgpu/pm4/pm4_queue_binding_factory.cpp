...
public:
  ...
  QueueBindingCreateResult create_binding(const QueueRegistrationRequest &request) override {
    if (request.type != QueueType::Compute || request.packet_format != QueuePacketFormat::Pm4)
      return {};
    // PM4 currently advances only through explicit frontend notifications.
    // Accepting either polling mode would discard its publication source and
    // leave a queue that appears live but cannot discover new packets.
