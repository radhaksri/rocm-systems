...
public:
  ...
  /// @brief Topology used when configuration supplies no per-field override.
  GpuDiscoveryTopology topology_defaults;
};
...
/// @brief Apply explicitly configured fields over one generation's defaults.
[[nodiscard]] GpuDiscoveryTopology
resolved_discovery_topology(const GpuGenerationDescriptor &generation,
                            const config::KfdDiscoveryOverrides &overrides);

/// @brief Resolve a configured target into the topology shared by KFD and PCI discovery.
/// @returns false when no registered generation answers for the configured target.
[[nodiscard]] bool resolve_gpu_generation_topology(config::KfdDeviceConfig &device);
