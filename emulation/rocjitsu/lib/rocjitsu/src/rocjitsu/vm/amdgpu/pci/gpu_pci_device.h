...
public:
  ...
  /// @brief Wait for a register-backed SDMA queue to consume its submitted work.
  [[nodiscard]] bool synchronize_sdma_queue_for_test(uint32_t engine);
  ...
  /// @brief Queue a diagnostic interrupt for delivery by the simulation owner.
  /// @details External control threads use this instead of touching the
  /// interrupt ring directly, so diagnostic requests and device completions
  /// cannot race one another through the multi-step ring publication.
  /// @returns Whether the request was admitted for the current transport.
  [[nodiscard]] bool request_interrupt(const InterruptEntry &entry);
...
private:
  ...
  enum class InterruptRequestKind : uint8_t { Completion, Diagnostic };
  ...
  public:
    ...
    InterruptRequestKind kind = InterruptRequestKind::Diagnostic;
  };
  ...
  [[nodiscard]] bool enqueue_sdma_pci_effect(const std::shared_ptr<SdmaPciCallbackState> &state,
                                             const SdmaPciEffect &effect);
  [[nodiscard]] bool enqueue_interrupt(const InterruptEntry &entry, InterruptRequestKind kind);
  ...
  std::deque<InterruptRequest> interrupt_inbox_;
