...
void ConsumerCursorJournal::retire(uint64_t cursor, uint64_t published_cursor, GpuVmAccess access) {
  assert(!publication_access_);
  cursor_ = cursor;
  publication_cursor_ = published_cursor;
  initialized_ = true;
...
VmAccessOutcome ConsumerCursorJournal::publish() {
  ...
  const VmAccessOutcome outcome =
      publication_access_->atomic_store(address_, width_, publication_cursor_);
