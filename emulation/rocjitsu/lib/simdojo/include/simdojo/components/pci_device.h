...
/// @brief Result of one indivisible 2-, 4-, or 8-byte DMA load.
...
protected:
  /// @brief Revoke and drain the current peer generation without releasing the
  ///        transport's ownership of this device.
  ///
  /// @details Reset uses this before destroying any state reachable through a
  /// retained operation lease. The transport remains attached so a reset that
  /// keeps the connection can publish a fresh generation afterwards.
  ///
  /// @returns true when this call revoked an active generation and the caller
  ///          may publish a replacement after reset.
  [[nodiscard]] bool quiesce_transport_session() {
    std::shared_ptr<PciTransportSession> closing;
    bool revoked_active = false;
    {
      std::lock_guard lock(transport_mutex_);
      ...
      closing = closing_transport_session_;
    }
    drain_transport_session(std::move(closing));
    return revoked_active;
  }

  /// @brief Publish a fresh peer generation after a successful reset.
  /// @returns true when a transport still owns the device and a new session was
  ///          installed, or when no transport is attached.
  [[nodiscard]] bool restart_transport_session_after_reset() {
    std::lock_guard lock(transport_mutex_);
    if (transport_ == nullptr)
      return true;
    if (transport_session_ != nullptr || closing_transport_session_ != nullptr)
      return false;
    transport_session_ = make_transport_session_locked(*transport_);
    return true;
  }
