...
The `ncclProfileNetPlugin` events emitted by the built-in IB and socket transports are compiled out
unless the build is configured with `-DENABLE_NET_PROFILING=ON`, because the instrumentation sits in
the network data path. Without it, no `NET_IB` or `NET_SOCK` events are reported no matter what the
event mask requests.
