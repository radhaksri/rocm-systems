...
TEST(core_api_test, descriptor_reports_correct_metadata)
{
    ...
    EXPECT_EQ(k_domain.meta.name, "hsa_core_api");
...
// Regression guard: hsa::core_api must push/pop timemory and stamp buffer-storage
// records with "rocm_hsa_api", not "rocm_hip_api" or any other domain's category.
TEST(core_api_test, uses_rocm_hsa_api_category)
{
    constexpr const auto& k_domain =
        k_core_api<mock_sdk_with_tracing, externals_with_tracing>;
